...
const LMS = {
  get apiBase(){ try{ return String((window.LTI_CONFIG && window.LTI_CONFIG.LMS_API_BASE) || '').replace(/\/$/, '') }catch{ return '' } },
  ...
  async api(path, opts={}){
    const r = await fetch(this.apiBase + path, {headers:{'Content-Type':'application/json',...(this.token?{Authorization:'Bearer '+this.token}:{})},...opts});
